"""
gpt_oss_counterexample_first — Solo solver for OpenAI gpt-oss-120b.

Strategy: spend more CPU on finite counterexample search before asking
gpt-oss-120b for a proof. This is intended for problem sets with many false
implications where a verified Fin table is cheaper and more reliable than
several model/judge proof attempts.
...
PROMPT = """You are a Lean 4 proof engineer using OpenAI gpt-oss-120b. Output ONLY valid JSON.
...
7. The solver already tried an aggressive finite search. If you answer FALSE, provide a concrete table that satisfies the hypothesis and violates the goal; do not guess.
...
def main():
    ...
    n, table = extended_counterexample(eq1_text, eq2_text, max_n=6, random_attempts=16000)
    if n is not None:
        result = call_judge("false", make_false_code(problem, n, table))
        if result.get("status") == "accepted":
            return
    ...
    n, table = backtrack_counterexample(eq1_text, eq2_text, sizes=(4, 5, 6), time_limit=22)
    if n is not None:
        result = call_judge("false", make_false_code(problem, n, table))
        if result.get("status") == "accepted":
            return
    ...
    notes.append("No counterexample found after aggressive Fin 2-6 search plus structured/random Fin-family probes")
    ...
    while True:
        ...
        # Temperature strategy: proof mode starts conservative after aggressive finite search.
        temps = [0.12, 0.22, 0.4, 0.65, 0.85]
