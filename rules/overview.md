...
- **Marathon** — N problems per solver subprocess (reference N=100), one shared global budget = `compression_ratio × N × Marathon per-problem reference (600 s)` (default `compression_ratio = 0.5`), file-based manifest in / append-only JSONL out.
