...
REPO = Path("/Users/nhatminh/dev/active/MagmaBallz")
...
# Corpora live either in the working tree (main) or only on the prototype
# branch (the evaluation_* sets); stage_corpora() materializes the latter.
CORPUS_DIR = SCRATCH / "corpora"
...
def run_one(sub: str, problem: dict, config: dict) -> dict:
    ...
    statuses = []
    lemma_certs = 0
    # Route only ever reaches us through the solver's stderr, which the proxy
    # keeps as a bounded tail. Recording it costs nothing and is what was
    # missing on 2026-08-21, when "which route won this problem?" could not be
    # answered from an 11-hour sweep's output and had to be re-measured.
    stderr_bits: list[str] = []
    for entry in result.get("log", []):
        # Đúng khóa là entry["tail"] dưới type "solver_stderr" (proxy.py:1099).
        # Bản đầu dò entry["stderr"] nên bắt trượt hoàn toàn — sweep 21/08 ghi
        # stderr_tail rỗng ở mọi dòng, đúng ca cần nhất thì câm.
        blob = (entry.get("tail") if entry.get("type") == "solver_stderr" else None) \
            or entry.get("stderr") or (entry.get("response") or {}).get("stderr")
        if blob:
            stderr_bits.append(str(blob))
    for entry in result.get("log", []):
        ...
        # critical-pair engine fingerprint: emitted lemma preamble
        if "have lem" in str((entry.get("request") or {}).get("code", "")):
            lemma_certs += 1
    return {
        "solver": sub,
        "id": problem["id"],
...
def main() -> None:
    ...
    # Optional LLM endpoint override (the repo default is OpenRouter; an
    # sk-proj OpenAI key 401s there). Non-OpenRouter base_url also disables
    # the proxy's OpenRouter-only extra_body fields automatically.
    if os.environ.get("SB_LLM_BASE_URL"):
        config["llm"]["base_url"] = os.environ["SB_LLM_BASE_URL"]
    if os.environ.get("SB_LLM_MODEL"):
        config["llm"]["model"] = os.environ["SB_LLM_MODEL"]
    if os.environ.get("SB_LLM_MAX_TOKENS"):
        config["llm"]["max_output_tokens"] = int(os.environ["SB_LLM_MAX_TOKENS"])
    if os.environ.get("SB_SANDBOX_MODE"):
        config["sandbox"]["mode"] = os.environ["SB_SANDBOX_MODE"]
    if "SB_LLM_PROVIDER" in os.environ:
        # empty string removes the pinned provider route entirely (needed when
        # the model isn't hosted by the config's default provider)
        if os.environ["SB_LLM_PROVIDER"]:
            config["llm"]["provider"] = os.environ["SB_LLM_PROVIDER"]
        else:
            config["llm"].pop("provider", None)
